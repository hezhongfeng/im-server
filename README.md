...
2. 用户和会话的关系是多对多的关系，一个会话可以包含多个用户（包含2个就是单聊），用户也可以包含多个会话（就是我们的会话列表）
3. 群组和会话之间的关系是1对1，可以通过群组找到此群组的会话，也可以通过会话确定群组（前提是这个会话是群组会话）

这样的话就可以保证，无论是单聊还是群聊，连接上的用户只要join进对应的会话就可以了，消息也是在对应的room里面发布，消息列表的保存和查询都比较简单，还有每个人的会话列表的维护也变得很简单
